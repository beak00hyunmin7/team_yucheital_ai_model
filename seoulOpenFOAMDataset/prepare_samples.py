"""
학습 데이터셋용 후보 지형 N개를 미리 골라서(완만한 곳만 필터링) 각 샘플 폴더에
terrain_raw.npy + params.json(강우 조건, 권장 시뮬레이션 시간)을 저장해둔다.
실제 OpenFOAM 실행은 이 산출물을 갖고 run_batch.sh(WSL)에서 순차적으로 진행한다.
"""
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "seoulTerrain"))
...
def pick_and_prepare(n_samples, max_slope=0.35, seed=0, max_tries=2000):
    ...
    while len(accepted) < n_samples and tries < max_tries:
        ...
        rain_mm = float(rng.uniform(20.0, 80.0))  # 설계강우 등가 총량 [mm]
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    pick_and_prepare(args.n, seed=args.seed)
