...
> **정정 (2026-09-16).** 위 수치는 재현되지 않습니다. `scripts/evaluate.py` 로 val 87지형
> 911장을 OpenFOAM 원본 depth 와 비교하면 **MAE 0.0110 m, r 0.684, 강우 단조성 98.8%**
> 입니다(`eval_v5/summary.md`). 보고서에는 이 실측값을 쓰세요.
> 또한 현재 권장 모델은 v5가 아니라 **`checkpoints_p2_wet8`** 입니다 - 1/5 크기로
> 최대수심 비율 0.51->0.62, IoU@5cm 0.378->0.426. 근거는 `IMPROVEMENT_GUIDE.md`.
...
> **정정 (2026-09-16).** 아래 "일반 조회엔 120초" 권장은 **틀렸습니다.** 학습 데이터는
> 비가 초반에 내리고 이후 빠지는 시뮬레이션이라 수심이 시간에 따라 **줄어듭니다**
> (val 정답 평균 수심 t=18s 0.039m -> t=120s 0.019m). `time_s=120`은 가장 덜 잠긴
> 상태입니다. "이 지형이 얼마나 잠기나"를 묻는 일반 조회에는 **`time_s=18.0` 부근**을
> 쓰세요. 웹앱 기본값도 18초입니다.
